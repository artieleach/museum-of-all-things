...
func _teleport_single_player(player, from_hall, to_hall, rot_diff: float) -> void:
	var diff_from = player.global_position - from_hall.position
...
func _teleport_all_network_players(to_hall, rot_diff: float) -> void:
...
func _on_loader_body_entered(body, hall, backlink=false):
	...
	if body.is_in_group("Player"):
		...
		if _is_multiplayer_active() and not _is_local_player(body):
			return

		if _is_multiplayer_active():
			# In multiplayer, request the transition from the host
			_request_multiplayer_transition(hall, backlink)
		else:
...
func _is_local_player(body) -> bool:
...
func _request_multiplayer_transition(hall, backlink: bool) -> void:
	...
	if NetworkManager.is_server():
		...
		request_transition.rpc_id(1, hall.to_title, hall_info)
...
func request_transition(to_title: String, hall_info: Dictionary) -> void:
	...
	var from_title = hall_info.get("from_title", _current_room_title)

	# Authorize and broadcast the transition to all clients
	execute_transition.rpc(to_title, from_title, hall_info)
...
func _execute_multiplayer_transition(to_title: String, from_title: String, hall_info: Dictionary) -> void:
	...
	var backlink = hall_info.get("backlink", false)

	if OS.is_debug_build():
		print("Museum: Executing multiplayer transition to ", to_title, " backlink=", backlink)

	# Find the hall that matches this transition
	var hall = _find_hall_for_transition(to_title, from_title, backlink)
	...
	get_tree().create_timer(0.5).timeout.connect(func(): _transition_in_progress = false)

func _find_hall_for_transition(to_title: String, from_title: String, backlink: bool):
	# Search through exhibits to find the matching hall
	if _exhibits.has(from_title):
		var exhibit_data = _exhibits[from_title]
		var exhibit = exhibit_data.get("exhibit")
		if is_instance_valid(exhibit) and exhibit.has_method("get") or "exits" in exhibit:
			if "exits" in exhibit:
	...
	# Check the lobby
	if from_title == "$Lobby" and is_instance_valid($Lobby):
...
func sync_to_exhibit(exhibit_title: String) -> void:
	...
	if exhibit_title == "$Lobby" or exhibit_title == _current_room_title:
		return

	if OS.is_debug_build():
		print("Museum: Syncing to exhibit ", exhibit_title)
