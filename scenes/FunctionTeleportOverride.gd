...
const DEFAULT_MASK: int = 0b1111_1111_1111_1111_1111_1111_1111_1111

# gdlint:ignore = load-constant-name
const _DefaultMaterial: StandardMaterial3D = preload("res://addons/godot-xr-tools/materials/capsule.tres")
...
## Teleport texture
@export var arc_texture: Texture2D = preload("res://addons/godot-xr-tools/images/teleport_arrow.png"): set = set_arc_texture
...
var collision_shape: CapsuleShape3D = null
...
@onready var player_body: XRToolsPlayerBody = XRToolsPlayerBody.find_instance(self)
...
func is_xr_class(p_name: String) -> bool:
	return p_name == "XRToolsFunctionTeleport"
...
func _physics_process(delta: float) -> void:
	...
	var new_ws: float = XRServer.world_scale
	if ws != new_ws:
		ws = new_ws
		$Teleport.mesh.size = Vector2(0.05 * ws, 1.0)
		$Target.mesh.size = Vector2(ws, ws)
	...
	if controller and controller.get_is_active() and controller.is_button_pressed(teleport_button_action):
		...
		var target_global_origin: Vector3 = teleport_global_transform.origin
		var up: Vector3 = player_body.up_player
		var down: Vector3 = -up.normalized() / ws
		...
		var cast_length: float = 0.0
		var fine_tune: float = 1.0
		var hit_something: bool = false
		var max_slope_cos: float = cos(deg_to_rad(max_slope))
		for i: int in range(1, step_count):
			var new_cast_length: float = cast_length + (step_size / fine_tune)
			var global_target: Vector3 = Vector3(0.0, 0.0, -new_cast_length)

			# our quadratic values
			var t: float = global_target.z / strength
			var t2: float = t * t
			...
			var cast_result: Array[Vector3] = state.collide_shape(query, 10)
			if cast_result.is_empty():
				...
				var collided_at: Vector3 = target_global_origin

				# check for collision
				var step_delta: Vector3 = global_target - target_global_origin
				if up.dot(step_delta) > 0:
					...
					var intersects: Dictionary = state.intersect_ray(ray_query)
					if intersects.is_empty():
						is_on_floor = false
					else:
						# did we collide with a floor or a wall?
						floor_normal = intersects["normal"]
						var dot: float = up.dot(floor_normal)
		...
		if hit_something:
			var color: Color = can_teleport_color
		...
		if can_teleport:
			...
			var new_transform: Transform3D = last_target_transform
